...
export class ModalCriarGrupoComponent {
  ...
  @Output() created = new EventEmitter<{ nome: string; descricao: string; moeda: string }>();
  ...
  criar(): void {
    ...
    this.created.emit({ nome: this.nome.trim(), descricao: this.descricao.trim(), moeda: this.moeda });
    this.close.emit();
  }
}
